...
<?php
  ...
  $restaurant_cafeteria_video = false;

  // Only get video from the content if a playlist isn't present.
  if ( false === strpos( $restaurant_cafeteria_content, 'wp-playlist-script' ) ) {
    $restaurant_cafeteria_video = get_media_embedded_in_content( $restaurant_cafeteria_content, array( 'video', 'object', 'embed', 'iframe' ) );
  }
?>
  ...
  <article id="post-<?php the_ID(); ?>" <?php post_class('article-box'); ?>>
    <?php
      if ( ! is_single() ) {
        ...
        if ( ! empty( $restaurant_cafeteria_video ) ) {
          foreach ( $restaurant_cafeteria_video as $restaurant_cafeteria_video_html ) {
            echo '<div class="entry-video">';
              echo $restaurant_cafeteria_video_html;
            echo '</div>';
          }
        };
      };
